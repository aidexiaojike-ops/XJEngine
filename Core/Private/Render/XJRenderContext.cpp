...
namespace XJ
{
    ...
    {
        mInstance = std::make_unique<XJVulkanInstance>();//创建Vulkan实例对象
        mSurface = std::make_unique<XJVulkanSurface>(mWindow, mInstance.get());//创建表面对象
        mPhysicalDevices = std::make_unique<XJVulkanPhysicalDevices>(mInstance.get(), mSurface.get());//创建物理设备对象
