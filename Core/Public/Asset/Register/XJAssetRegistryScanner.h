...
namespace XJ
{
    ...
    struct XJAssetRegistryScanReport//注册表扫描报告
    {
        uint32_t Added = 0;
        uint32_t Updated = 0;
    ...
    class XJAssetRegistryScanner
    {
        public:
            ...
            static XJAssetRegistryScanReport ScanResourceAssetsDetailed(XJAssetRegistry& registry, const std::filesystem::path& resourceRoot);
            static XJAssetType GetAssetTypeFromExtension(const std::filesystem::path& path);//根据文件扩展名判断资产类型
            static XJAssetHandle GenerateStableHandle(const std::filesystem::path& path, XJAssetType type, uint32_t collisionSalt = 0);//根据文件路径和类型生成一个稳定的 handle，确保同一文件每次扫描得到的 handle 都相同
