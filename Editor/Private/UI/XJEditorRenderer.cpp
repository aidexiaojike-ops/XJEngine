...
namespace XJ
{
    bool XJEditorRenderer::Init(const XJEditorRendererInitInfo& info)
    {
        ...
        //// 🔥 核心：新版写法
        kInitInfo.PipelineInfoMain.RenderPass = info.renderPass;
        kInitInfo.PipelineInfoMain.Subpass = info.subpass;
        kInitInfo.PipelineInfoMain.MSAASamples = info.msaaSamples;


           // ⭐ 新增（新版关键）  // ⭐ Dynamic Rendering 配置
        kInitInfo.UseDynamicRendering = false;
