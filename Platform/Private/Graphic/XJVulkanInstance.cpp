...
namespace XJ
{
    ...
    XJVulkanInstance::XJVulkanInstance()
    {
       //构建支持的层 获取当前系统（显卡驱动 + 操作系统）支持的所有「Vulkan 实例层」的列表和详细信息。
        ...
        do
        {
            ...
            {
                XJDebug_Log(result);
                throw std::runtime_error("XJVulkanInstance failed: enumerate instance layers failed");
            }
        }
        ...
        if(bShouldValidate)
        {
                ...
                enableLayers.data()))
            {
                ...
                throw std::runtime_error("XJVulkanInstance failed: missing required instance layers");
            }
        ...
        do
        {
            ...
            {
                XJDebug_Log(result);
                throw std::runtime_error("XJVulkanInstance failed: enumerate instance extensions failed");
            }
        }
        while (result == VK_INCOMPLETE);
        ...
        if (!glfwRequestedExtensions || glfwRequestedExtensionCount == 0)
        {
            spdlog::error("GLFW 未返回 Vulkan 实例必需扩展，请确认 GLFW 已初始化且当前平台支持 Vulkan");
            throw std::runtime_error("XJVulkanInstance failed: glfwGetRequiredInstanceExtensions returned null");
        }
            ...
            enableExtensions.data()))
        {
            ...
            throw std::runtime_error("XJVulkanInstance failed: missing required instance extensions");
        }
        ...
        if (result != VK_SUCCESS)
        {
            throw std::runtime_error("XJVulkanInstance failed: vkCreateInstance failed");
        }
        ...
        if (bShouldValidate)
        {
            ...
            if (func != nullptr)
            {
                ...
                if (result != VK_SUCCESS)
                {
                    ...
                    mInstance = nullptr;
                    throw std::runtime_error("XJVulkanInstance failed: vkCreateDebugUtilsMessengerEXT failed");
                }
            ...
            else
            {
                vkDestroyInstance(mInstance, nullptr);
                mInstance = nullptr;
                throw std::runtime_error("XJVulkanInstance failed: vkCreateDebugUtilsMessengerEXT not found");
            }
        }
    }
