...
namespace XJ
{
    ...
    class XJVulkanBuffer
    {
        ...
        public:
            ...
            static void CopyToBuffer(XJVulkanDevice* device, VkBuffer srcBuffer, VkBuffer dstBuffer, size_t size);
            
            VkResult WriteData(const void* data);//写入数据到缓冲区

            VkBuffer XJGetBuffer() const { return mBuffer; }

            VkResult WriteDataOffset(const void *data, size_t offset, size_t size);//写入数据到缓冲区的指定偏移位置
        };
    }
