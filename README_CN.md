...
| **Shader Schema 系统** | JSON 定义着色器参数（color、float、texture2D），支持默认值和范围，材质资产序列化 |
...
- **Shader Schema 系统**：JSON 定义的着色器参数（`Unlit.schema`），类型包括 color4、float、texture2D，支持默认值、最小值/最大值
